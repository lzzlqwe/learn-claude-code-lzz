...
**`/resume` 恢复历史会话**：会话按项目隔离落盘，随时可以恢复。会话内输入 `/resume` 打开会话浏览器选择；命令行 `qoder -r <session-id>` 恢复指定会话，`qoder -c` 直接继续最近一次。跨天的长任务不用重新交代背景，接着上次的进度继续。

**小技巧：用 HANDOFF.md 交接长任务**。长任务快跑到上下文尽头时，让它先写一份 `HANDOFF.md`：目标、已经试过什么、什么有效什么无效、下一步。新会话只需要给这个文件路径就能接上。这比压缩可控：压缩丢什么不由你决定，交接文档丢什么由你决定。
